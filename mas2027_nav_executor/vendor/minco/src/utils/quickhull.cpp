...
template <typename T> ConvexHull<T>::ConvexHull(const ConvexHull & o)
{
...
template <typename T> ConvexHull<T>::ConvexHull(ConvexHull<T> && o)
{
...
ConvexHull<T>::ConvexHull(
  const MeshBuilder<T> & mesh, const VertexDataSource<T> & pointCloud, bool CCW, bool useOriginalIndices)
{
...
template <typename T> std::vector<size_t> & ConvexHull<T>::getIndexBuffer()
{
  return m_indices;
}

template <typename T> const std::vector<size_t> & ConvexHull<T>::getIndexBuffer() const
{
  return m_indices;
}

template <typename T> VertexDataSource<T> & ConvexHull<T>::getVertexBuffer()
{
  return m_vertices;
}

template <typename T> const VertexDataSource<T> & ConvexHull<T>::getVertexBuffer() const
{
  return m_vertices;
}
...
template <typename T> ConvexHull<T> & ConvexHull<T>::operator=(const ConvexHull<T> & o)
{
  if (&o == this) {
    return *this;
  }
...
template <typename T> ConvexHull<T> & ConvexHull<T>::operator=(ConvexHull<T> && o)
{
  if (&o == this) {
    return *this;
  }
...
HalfEdgeMesh<T, IndexType>::HalfEdgeMesh(
  const MeshBuilder<T> & builderObject, const VertexDataSource<T> & vertexData)
{
...
template <> float defaultEps()
{
  return 0.0001f;
}

template <> double defaultEps()
{
  return 0.0000001;
}
...
ConvexHull<T> QuickHull<T>::getConvexHull(
  const VertexDataSource<T> & pointCloud, bool CCW, bool useOriginalIndices, T epsilon)
{
...
template <typename T>
ConvexHull<T> QuickHull<T>::getConvexHull(
  const std::vector<Vector3<T>> & pointCloud, bool CCW, bool useOriginalIndices, T epsilon)
{
...
template <typename T>
ConvexHull<T> QuickHull<T>::getConvexHull(
  const Vector3<T> * vertexData, size_t vertexCount, bool CCW, bool useOriginalIndices, T epsilon)
{
...
template <typename T>
ConvexHull<T> QuickHull<T>::getConvexHull(
  const T * vertexData, size_t vertexCount, bool CCW, bool useOriginalIndices, T epsilon)
{
...
HalfEdgeMesh<FloatType, size_t> QuickHull<FloatType>::getConvexHullAsMesh(
  const FloatType * vertexData, size_t vertexCount, bool CCW, FloatType epsilon)
{
  ...
  const VertexDataSource<T> & pointCloud, bool CCW, bool useOriginalIndices, T epsilon)
{
  ...
  m_planar = false;  // The planar case happens when all the points appear to lie on a two dimensional
                     // subspace of R^3.
...
template <typename T> void QuickHull<T>::createConvexHalfEdgeMesh()
{
  ...
  while (!m_faceList.empty()) {
    ...
    if (iter == std::numeric_limits<size_t>::max()) {
      // Visible face traversal marks visited faces with iteration counter (to mark that the face has been
      // visited on this iteration) and the max value represents unvisited faces. At this point we have to
      // reset iteration counter. This shouldn't be an issue on 64 bit machines.
      iter = 0;
    }
    ...
    // The face on top of the stack of course is one of them. At the same time, we create a list of horizon
    // edges.
    m_horizonEdges.clear();
    m_possiblyVisibleFaces.clear();
    m_visibleFaces.clear();
    ...
    while (m_possiblyVisibleFaces.size()) {
      ...
      // Store which half edge is the horizon edge. The other half edges of the face will not be part of the
      // final mesh so their data slots can by recycled.
    ...
    // Order horizon edges so that they form a loop. This may fail due to numerical instability in which
    // case we give up trying to solve horizon edge for this point and accept a minor degeneration in the
    // convex hull.
    ...
    // Except for the horizon edges, all half edges of the visible faces can be marked as disabled. Their
    // data slots will be reused. The faces will be disabled as well, but we need to remember the points
    // that were on the positive side of them - therefore we save pointers to them.
    ...
    for (auto faceIndex : m_visibleFaces) {
      ...
      // Disable the face, but retain pointer to the points that were on the positive side of it. We need to
      // assign those points to the new faces we create shortly.
...
template <typename T> bool QuickHull<T>::reorderHorizonEdges(std::vector<size_t> & horizonEdges)
{
...
template <typename T> T QuickHull<T>::getScale(const std::array<size_t, 6> & extremeValues)
{
  T s = 0;
...
template <typename T> void QuickHull<T>::setupInitialTetrahedron()
{
  ...
  if (maxD == m_epsilonSquared) {
    // It appears that the point cloud belongs to a 1 dimensional subspace of R^3: convex hull has no volume
    // => return a thin triangle Pick any point other than selectedPoints.first and selectedPoints.second as
    ...
    const size_t thirdPoint =
      (it == m_vertexData.end()) ? selectedPoints.first : std::distance(m_vertexData.begin(), it);
  ...
  const Vector3<T> baseTriangleVertices[] = {
    m_vertexData[baseTriangle[0]], m_vertexData[baseTriangle[1]], m_vertexData[baseTriangle[2]]};

  // Next step is to find the 4th vertex of the tetrahedron. We naturally choose the point farthest away
  // from the triangle plane.
  maxD = m_epsilon;
  maxI = 0;
  const Vector3<T> N =
    mu::getTriangleNormal(baseTriangleVertices[0], baseTriangleVertices[1], baseTriangleVertices[2]);
  ...
  if (maxD == m_epsilon) {
    // All the points seem to lie on a 2D subspace of R^3. How to handle this? Well, let's add one extra
    // point to the point cloud so that the convex hull will have volume.
    m_planar = true;
    const vec3 N1 =
      mu::getTriangleNormal(baseTriangleVertices[1], baseTriangleVertices[2], baseTriangleVertices[0]);
    m_planarPointCloudTemp.clear();
  ...
  // Enforce CCW orientation (if user prefers clockwise orientation, swap two vertices in each triangle when
  // final mesh is created)
  ...
  // Finally we assign a face for each vertex outside the tetrahedron (vertices inside the tetrahedron have
  // no role anymore)
  for (size_t i = 0; i < vCount; i++) {
...
template <typename T> std::unique_ptr<std::vector<size_t>> QuickHull<T>::getIndexVectorFromPool()
{
...
void QuickHull<T>::reclaimToIndexVectorPool(std::unique_ptr<std::vector<size_t>> & ptr)
{
  ...
  if ((oldSize + 1) * 128 < ptr->capacity()) {
    // Reduce memory usage! Huge vectors are needed at the beginning of iteration when faces have many
    // points on their positive side. Later on, smaller vectors will suffice.
    ptr.reset(nullptr);
    return;
  }
...
template <typename T> size_t MeshBuilder<T>::addFace()
{
...
template <typename T> size_t MeshBuilder<T>::addHalfEdge()
{
...
template <typename T> std::unique_ptr<std::vector<size_t>> MeshBuilder<T>::disableFace(size_t faceIndex)
{
...
template <typename T> void MeshBuilder<T>::disableHalfEdge(size_t heIndex)
{
...
template <typename T> void MeshBuilder<T>::setup(size_t a, size_t b, size_t c, size_t d)
{
  m_faces.clear();
  m_halfEdges.clear();
...
template <typename T> std::array<size_t, 3> MeshBuilder<T>::getVertexIndicesOfFace(const Face & f) const
{
...
template <typename T> std::array<size_t, 3> MeshBuilder<T>::getHalfEdgeIndicesOfFace(const Face & f) const
{
  return {f.m_he, m_halfEdges[f.m_he].m_next, m_halfEdges[m_halfEdges[f.m_he].m_next].m_next};
}

template <typename T> void MeshBuilder<T>::Face::disable()
{
  m_he = std::numeric_limits<size_t>::max();
}

template <typename T> bool MeshBuilder<T>::Face::isDisabled() const
{
  return m_he == std::numeric_limits<size_t>::max();
}

template <typename T> void MeshBuilder<T>::HalfEdge::disable()
{
  m_endVertex = std::numeric_limits<size_t>::max();
}

template <typename T> bool MeshBuilder<T>::HalfEdge::isDisabled() const
{
...
template class HalfEdgeMesh<double, size_t>;

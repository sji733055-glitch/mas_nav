...
双雷达时仍是这两条话题：`enable_lidar_merge: true` 后两台 MID360 在 `mid360_driver` 里按源 IP 配对，后雷达变到 `lidar_link` 再发出。硬件、PTP、外参见 [dual-lidar.md](dual-lidar.md)。默认关融合；`is_topic_name_with_lidar_ip: false` 时插第二台会把两路点无外参拼进同一发布器。

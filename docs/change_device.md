‼️ **2026-09-22 架构更新**：本文对 `/dynamic_cost_map`、`bypass_dynamic_obstacle`
和二维动态层的分析只保留为历史排障记录，不再是当前配置指南。主导航栈已删除
这一整条依赖：全局搜索使用静态 terrain，在线障碍由 ROGMap 的局部绕行、轨迹监视
和指令净空门负责。当前操作以 [`README.md`](../README.md) 为准。
